...
namespace KurumsalWeb.Controllers
{
    public class HizmetController : Controller
    {
       ...
       public ActionResult Create(Hizmet hizmet,HttpPostedFileBase ResimURL)
        ...
        public ActionResult Edit(int? id)//boş olabilir diye belirtmek için ? koyduk
        {
            if(id==null)
            {
        ...
        public ActionResult Edit(int? id,Hizmet hizmet,HttpPostedFileBase ResimURL)
        {
            if(ModelState.IsValid)
            {
                ...
                if (ResimURL !=null)
                {
                    ...
                    if (!string.IsNullOrEmpty(h.ResimURL) && System.IO.File.Exists(Server.MapPath(h.ResimURL)))
                    {
