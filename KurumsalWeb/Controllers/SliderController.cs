        ...
        public ActionResult Create([Bind(Include = "SliderId,Baslik,Aciklama,ResimURL")] Slider slider,HttpPostedFileBase ResimURL)
        {
            if (ModelState.IsValid)
            {
                ...
                if (ResimURL != null)
                {
                    ...
                    string sliderimgname = Guid.NewGuid().ToString() + imginfo.Extension;//otomatik resmi isimlendirecek
                    img.Resize(1024, 360);
        ...
        public ActionResult Edit([Bind(Include = "SliderId,Baslik,Aciklama,ResimURL")] Slider slider,HttpPostedFileBase ResimURL,int id)
        {
            if (ModelState.IsValid)
            {
                ...
                if (ResimURL != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(s.ResimURL))
                    {
                        var oldFilePath = Server.MapPath("~/" + s.ResimURL);
                    ...
                    // string logoname = LoginURL.FileName + imginfo.Extension; //masaüstüne nasıl kaydetmişsek o şekilde veritabanına kayıt olacak
                    string sliderimgname = ResimURL.FileName;
                    img.Resize(1024, 360);
                    img.Save("~/Uploads/Slider/" + sliderimgname);
